...
class Babbler:
    ...
    def add_sentence(self, sentence):
        ...
        """
        sentence = sentence.strip().lower() # set to lower case
        words = sentence.split() # split the sentences
        ...
        for i in range(len(words) - self.n): # get the length of the sentence
    ...
    def has_successor(self, ngram):
        ...
        Probably a one-line method.
        """
        return ngram in self.brainGraph and bool(self.brainGraph[ngram])
    ...
    def get_random_successor(self, ngram):
        ...
        successors = self.brainGraph.get(ngram, [])
        if successors:
    ...
    def babble(self):
        ...
        while True:
            next_word = self.get_random_successor(' '.join(sentence[-self.n:]))
